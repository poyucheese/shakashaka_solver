...
static bool build_solution_path(const char *input_path, char *output_path, size_t output_size) {
    ...
    if (last_dot == NULL) {
        return snprintf(output_path, output_size, "%s_solved.txt", input_path) < (int)output_size;
    }

    return snprintf(output_path, output_size, "%.*s_solved.txt", (int)prefix_length, input_path) < (int)output_size;
}
...
static bool run_logic_until_stable(int **board, int height, int width, int *unsolved, int *iteration_out) {
    bool update = true;
                ...
                if (board[row][col] >= BLACK_0 && board[row][col] <= BLACK_4) {
                    ...
                    if (triangles == required_triangles) {
                        ...
                        if (board[row][col - 1] & CAND_WHITE) {
                            ...
                            if (board[row][col - 1] == 0) return false;
                            update = true;
                        }
                        if (board[row][col + 1] & CAND_WHITE) {
                            board[row][col + 1] &= ~CAND_WHITE;
                    ...
                    if (board[row][col] >= WHITE && board[row][col] <= TRI_DOWN_RIGHT) {
                        (*unsolved)--;
                    }
                    if (board[row][col] != previous_state) {
                        update = true;
                    }
                }
            }
            ...
            for (int row = 1; row <= height; row++) {
                for (int col = 1; col <= width; col++) {
                    if (board[row][col] >= BLACK_0 && board[row][col] <= BLACK_4) {
                        ...
                        if (triangles != required_triangles || triangles + not_triangles != 4) {
                            return false;
                        }
                    }
                    ...
                    else if (board[row][col] >= TRI_UP_LEFT && board[row][col] <= TRI_DOWN_RIGHT) {
                        if (board[row][col] == TRI_UP_LEFT) {
                            if (has_black_edge_toward(board[row + 1][col], EDGE_UP) ||
                                has_black_edge_toward(board[row][col + 1], EDGE_LEFT) ||
                                board[row + 1][col] == TRI_DOWN_RIGHT ||
                                board[row][col + 1] == TRI_DOWN_RIGHT ||
                                board[row - 1][col] == TRI_UP_LEFT ||
                            ...
                            if (has_black_edge_toward(board[row - 1][col], EDGE_DOWN) ||
                                has_black_edge_toward(board[row][col + 1], EDGE_LEFT) ||
                                board[row - 1][col] == TRI_UP_RIGHT ||
                                board[row][col + 1] == TRI_UP_RIGHT ||
                                board[row + 1][col] == TRI_DOWN_LEFT ||
...
static bool find_first_candidate_cell(int **board, int height, int width, int *row_out, int *col_out) {
    for (int row = 1; row <= height; row++) {
        for (int col = 1; col <= width; col++) {
            if (is_candidate_cell(board[row][col])) {
                *row_out = row;
                *col_out = col;
                return true;
            }
...
static int **clone_board(int **board, int board_height, int board_width) {
    int **clone = (int **)malloc((size_t)board_height * sizeof(int *));
    if (clone == NULL) {
        return NULL;
    }

    for (int row = 0; row < board_height; row++) {
        clone[row] = (int *)malloc((size_t)board_width * sizeof(int));
        if (clone[row] == NULL) {
            free_board(clone, row);
            return NULL;
        }
        memcpy(clone[row], board[row], (size_t)board_width * sizeof(int));
    }

    return clone;
}

static bool solve_with_backtracking(int **board, int height, int width, int board_height, int board_width, int *unsolved, int *iteration_out) {
    const int candidates[] = {CAND_WHITE, CAND_TRI_UP_LEFT, CAND_TRI_UP_RIGHT, CAND_TRI_DOWN_LEFT, CAND_TRI_DOWN_RIGHT};
    int guess_row = 0;
    int guess_col = 0;
    ...
    if (!find_first_candidate_cell(board, height, width, &guess_row, &guess_col)) {
        return false;
    }

    int **snapshot = clone_board(board, board_height, board_width);
    if (snapshot == NULL) {
        fprintf(stderr, "Error: failed to allocate snapshot board.\n");
        return false;
    }

    int unsolved_before_guess = *unsolved;
    int cell_candidates = board[guess_row][guess_col] & CAND_ALL;

    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
        if ((cell_candidates & candidates[i]) == 0) {
            continue;
        }

        for (int row = 0; row < board_height; row++) {
            memcpy(board[row], snapshot[row], (size_t)board_width * sizeof(int));
        }
        *unsolved = unsolved_before_guess;

        board[guess_row][guess_col] = resolve_single_candidate(candidates[i]);
        (*unsolved)--;

        if (!run_logic_until_stable(board, height, width, unsolved, iteration_out)) {
            continue;
        }

        if (*unsolved == 0 || solve_with_backtracking(board, height, width, board_height, board_width, unsolved, iteration_out)) {
            free_board(snapshot, board_height);
            return true;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    int iteration = 0;
    if (!run_logic_until_stable(board, height, width, &unsolved, &iteration)) {
        fprintf(stderr, "Error: contradiction detected during logical solving.\n");
        free_board(board, board_height);
        return 1;
    }
    ...
    if (unsolved > 0 && !solve_with_backtracking(board, height, width, board_height, board_width, &unsolved, &iteration)) {
        fprintf(stderr, "Error: no valid solution found.\n");
        free_board(board, board_height);
        return 1;
    }
